...
## 3.1 Where Are Staged Files and Commits Stored?
...
Git stores the staged version in:
...
```text
-rw-r--r-- 1 user user 512 Jun 24 10:30 .git/index
```

### Commit Area (git commit)
...
```text
.git/objects/45/a12cde...
.git/objects/9f/8b1234...
.git/objects/ab/98ef45...
```
...
```mermaid
flowchart LR
    A["app.txt"] -->|git add| B[".git/index<br/>Staging Area"]
    B -->|git commit| C[".git/objects<br/>Commit Storage"]
...
|----------|------------|
| git add | .git/index |
| git commit | .git/objects/ |
| Branches | .git/refs/heads/ |
| HEAD | .git/HEAD |
| Remote URLs | .git/config |
...
### What Git Actually Does Internally
...
| git fetch | Downloads commits, branches and tags |
...
**Q: Does git clone only download files?**

No. It downloads the entire Git repository including commits, branches, tags and remote configuration.

**Q: Difference between git clone and git init?**

| git clone | git init |
|------------|-----------|
| Creates repository from existing remote | Creates empty repository |
| Downloads commit history | No history |
| Configures remote origin | No remote configured |
| Creates working files | Empty repository |
...
Using older/common command:

```bash
git checkout -b feature-payment
```

This command creates a new branch named `feature-payment` and immediately switches to that branch.
...
## 14.1 Merge Example – Different Files Modified (Automatic Merge)
...
gitGraph
    ...
    commit id: "app.txt changed"
    checkout main
    branch feature-db
    checkout feature-db
    commit id: "db.txt changed"
```
...
Because each branch modified a different file.

```mermaid
flowchart LR
    A[feature-app<br/>app.txt changed] --> C[Git Merge]
    B[feature-db<br/>db.txt changed] --> C
    C --> D[Automatic Merge Success]
...
## 14.2 Merge Example – Same File Modified (Merge Conflict)
...
```bash
git commit -am "App version update"
```
...
```bash
git commit -am "UI version update"
```
...
```text
Version 2 and Version 3 combined
```
...
```mermaid
flowchart TD
    A[main branch] --> C[Same File Modified]
    B[feature-ui branch] --> C
    C --> D[Merge Conflict]
    ...
    style E fill:#fef9c3,stroke:#ca8a04,color:#000
    style F fill:#e0f2fe,stroke:#0284c7,color:#000
...
```mermaid
flowchart LR
    A[Local Branches] --> B[git merge]
    B --> C[Local Repository Updated]
    C --> D[git push]
    D --> E[Remote Repository Updated]
...
**Does `git merge` happen on GitHub automatically?**

No. `git merge` normally happens in your local repository. The merged result reaches GitHub/GitLab only after a `git push`.
...
## 20.1 Real-World Example – Understanding Git Fetch vs Git Pull

### Initial Situation

Remote GitHub repository contains:
...
Your local repository also contains:
...
### Another Developer Updates Remote Repository
...
```mermaid
flowchart LR
    A[Local Repo<br/>Version 1] --> B[Remote Repo<br/>Version 2]
...
From github.com/company/project
   7a9c2d1..8b7e3f2  main -> origin/main
```
...
```mermaid
flowchart LR
    A[Remote Repo] -->|git fetch| B[origin/main Updated]
    B -.-> C[Local main Unchanged]
...
```mermaid
flowchart LR
    A[Remote Repo] -->|Fetch| B[origin/main]
    B -->|Merge| C[Local main Updated]
...
**Which is safer in production environments?**

`git fetch` is generally safer because you can review incoming changes before merging them into your local branch.
...
## 21.1 Real-World Rebase Example (File Modification)
...
### Step 2: Another Developer Updates Main
...
### Repository State Before Rebase
...
gitGraph
    ...
    branch feature-login
    checkout feature-login
    commit id: "Add Login Feature (Replayed)"
```
...
git rebase
  = replays my commits on latest branch
```
...
```mermaid
flowchart LR
    A["feature-login\nLogin Feature"] --> B["git rebase main"]
    C["main\nSecurity Patch"] --> B
    B --> D["Feature branch moved to latest main"]
    D --> E["Login commit replayed"]
...
**Why do teams use rebase before creating a Pull Request?**

To move feature branch commits on top of the latest main branch and maintain a clean linear commit history.
...
```mermaid
flowchart LR
    A[main: Commit A] --> B[main branch]
    A --> C[feature branch]
    C --> D[Commit F1]
    D --> E[Commit F2]
    D -. git cherry-pick F1 .-> F[main: F1 copied]
    B --> F
